...
static bool elf_check_file(Elf64_Ehdr *hdr) {
	...
	if(hdr->e_ident[EI_MAG0] != ELFMAG0) {
		dprintf(PREFIX "[ERR] " "ELF Header EI_MAG0 incorrect.\n");
		return false;
	}
	if(hdr->e_ident[EI_MAG1] != ELFMAG1) {
		dprintf(PREFIX "[ERR] " "ELF Header EI_MAG1 incorrect.\n");
		return false;
	}
	if(hdr->e_ident[EI_MAG2] != ELFMAG2) {
		dprintf(PREFIX "[ERR] " "ELF Header EI_MAG2 incorrect.\n");
		return false;
	}
	if(hdr->e_ident[EI_MAG3] != ELFMAG3) {
		dprintf(PREFIX "[ERR] " "ELF Header EI_MAG3 incorrect.\n");
		return false;
	}
	return true;
}
...
static bool elf_check_supported(Elf64_Ehdr *hdr) {
	if(!elf_check_file(hdr)) {
		dprintf(PREFIX "[ERR] " "Invalid ELF File.\n");
		return false;
	}
	if(hdr->e_ident[EI_CLASS] != ELFCLASS64) {
		dprintf(PREFIX "[ERR] " "Unsupported ELF File Class.\n");
		return false;
	}
	if(hdr->e_ident[EI_DATA] != ELFDATA2LSB) {
		dprintf(PREFIX "[ERR] " "Unsupported ELF File byte order.\n");
		return false;
	}
	if(hdr->e_machine != EM_AMD64) {
		dprintf(PREFIX "[ERR] " "Unsupported ELF File target.\n");
		return false;
	}
	if(hdr->e_ident[EI_VERSION] != EV_CURRENT) {
		dprintf(PREFIX "[ERR] " "Unsupported ELF File version.\n");
		return false;
	}
	if(hdr->e_type != ET_REL && hdr->e_type != ET_EXEC) {
		dprintf(PREFIX "[ERR] " "Unsupported ELF File type.\n");
		return false;
	}
	return true;
}
...
void ModuleManager::LoadELF(uint8_t *data) {
        ...
        if(elf_check_file(header) && elf_check_supported(header)) {
                dprintf(PREFIX "ELF file is valid!\n");

        	uint64_t size = header->e_phnum * header->e_phentsize;
                dprintf(PREFIX "File size: %d\n", size);
