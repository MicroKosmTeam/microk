...
public:
	...
	RAMFSDriver(const RAMFSDriver &original) {

	}

	void            FSInit();
	void            FSDelete();
	FSNode         *FSMakeFile(FSNode *node, const char *name, uint64_t uid, uint64_t gid, uint64_t mask);
	FILE           *FSOpenFile(FSNode *node, uint64_t descriptor);
	uint64_t        FSReadFile(FILE *file, uint64_t offset, size_t size, uint8_t **buffer);
	uint64_t        FSWriteFile(FILE *file, uint64_t offset, size_t size, uint8_t *buffer);
	void            FSCloseFile(FILE *file);
	uint64_t	FSDeleteFile(FSNode *node);
	FSNode         *FSMakeDir(FSNode *node, const char *name, uint64_t uid, uint64_t gid, uint64_t mask);
	FSNode         *FSReadDir(FSNode *node, uint64_t index);
	FSNode         *FSFindDir(FSNode *node, const char *name);
	uint64_t        FSGetDirElements(FSNode *node);
	uint64_t       *FSDeleteDir(FSNode *node);
private:
