...
FSNode *VFSMakeDir(VFilesystem *fs, FSNode *node, const char *name, uint64_t mask, uint64_t uid, uint64_t gid) {
	return fs->driver->FSMakeDir(node, name, mask, uid, gid);
}

FSNode *VFSMakeFile(VFilesystem *fs, FSNode *node, const char *name, uint64_t mask, uint64_t uid, uint64_t gid) {
...
void VFS_Init() {
	...
	FSNode *devdir = VFSMakeDir(rootfs, rootfs->node, "dev", 0, 0, 0);
	FSNode *sysdir = VFSMakeDir(rootfs, rootfs->node, "sys", 0, 0, 0);
	FSNode *procdir = VFSMakeDir(rootfs, rootfs->node, "proc", 0, 0, 0);
	...
	memset(buffer, 'A', 128);
	printk("Write.\n");
	VFSWrite(devtmpfs, tty, 0, 128, buffer);
	memset(buffer, 'B', 128);
	printk("Read.\n");
	VFSRead(devtmpfs, tty, 64, 64, &buffer);
	printk("\nA: %c\n", buffer[64]);
...
FSNode *VFSTraversePath(char *path) {
	VFilesystem *fs = rootfs;
	FSNode *node = rootfs->node;
	...
	do {
		node = VFSFindDir(fs, node, nodeName);

		if (node == NULL) return NULL;

		for (int i = 0; i < activeVFSs; i++) {
			VFilesystem *drive = drives[i]; // drives.Get(i);
			if(drive->mountdir->impl == node->impl &&
			   drive->mountdir->inode == node->inode &&
			   strcmp(drive->mountdir->name, node->name) == 0) {
				fs = drive;
				node = fs->node;
				break;
			}
		}
	} while ((nodeName = strtok(NULL, "/")) != NULL);
...
void VFS_Cat(char *path, char *name) {
	VFilesystem *fs = rootfs;
	FSNode *directory = rootfs->node;
	...
	FSNode *node = VFSFindDir(fs, directory, name);
	if (node == NULL) return;
	else printf("File found.\n");
	FILE *file = VFSOpen(fs, node, 1024);
	if (file == NULL) return;
	else printf("Done opening.\n");
	uint8_t *buffer = new uint8_t[128];
	while (true) {
		memset(buffer, '0', 128);
		uint64_t size = VFSRead(fs, file, 0, 128, &buffer);
		if (size == 0) return;
